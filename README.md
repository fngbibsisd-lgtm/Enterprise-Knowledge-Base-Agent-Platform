...
由脚本按 `trace` 里的工具参数**重放同一批调用**得到，连工具层的单条截断和 executor 的 4000 字截断一起复现。
...
**唯一稳定失败的一题 —— 根因在上下文预算，不在推理**：问「十二五规划中，化学需氧量和二氧化硫排放量要求分别减少多少」，
答案是 8%，该句在《十二五规划纲要》工具返回文本的**第 5068 字**处；而 executor 会把单条工具结果截到
`agent_tool_result_max_chars = 4000` 字，Agent 从未看到这句，于是老实答「未找到」。
裁判给的判定是**正确性 false / 忠实性 true**——它没编造，只是没答出来。两处与上下文预算相关的问题：

- `get_document_fn` 允许返回 16000 字（`agent_tool_result_max_chars * 4`），但 executor 的 4000 字截断
  让它永远拿不满，长文档"读全文"实际只有 2% 的可见度；
- `rag.search` 做了**文档级去重**（每个文档最多返回 1 个 chunk），大文档换个问法就可能恰好避开正确 chunk。
