...
基于 **RAG 混合检索 + 手写 Function Calling Agent** 的企业知识库问答系统，在同一个对话入口内同时提供制度文档检索与结构化数据查询两类能力。

上传 PDF/TXT 制度文档后，系统提供两种交互模式：

- **RAG 模式**：固定执行一次混合检索，答案附带 `[id]` 引用来源。
- **Agent 模式**：由 LLM 自主决定检索文档或查询数据库、以及工具调用轮数，前端实时展示工具调用轨迹。

系统面向银行与企业场景中的两类典型问题：「某项制度是如何规定的」与「当前有多少条记录」。

系统的核心是两件事：**混合检索的质量**（向量 + BM25 → RRF → 年份策略）与 **Agent 循环的自主性**（不依赖 LangChain/LangGraph，完整循环自行实现）。其余能力（MCP / 多智能体 / LangGraph 对照实现）均位于 `examples/`，属于可插拔扩展，不启动不影响主线。


## 阅读导航

- **部署与运行**：请先阅读 [Quick Start](#quick-start)；接口细节见 [API 接口](#api-接口)；遇到问题请参阅 [常见问题排查](#常见问题排查)。
- **功能与边界**：[主要功能](#主要功能) 与 [兼容状态](#兼容状态)。
...
- **后续开发**：[Future Work](#future-work) 按优先级列出了已明确但尚未实施的工作。
- **详细设计文档**：[docs/技术设计文档.md](./docs/技术设计文档.md)（3 万字，逐模块设计说明）。
...
- **混合检索**：Milvus 向量 + BM25（字符 bigram）→ RRF 融合 → 文档级去重 → 年份软排序；指定 `source` 精查某一份文档时跳过去重，并将候选池放大到全量。
- **RAG 问答**：检索 + LLM 生成，返回答案并附引用来源与相关度。
- **Agent 问答**：手写 Function Calling 循环，自主决定工具与调用轮数；SSE 流式输出状态、工具调用与答案。
- **工具集**：`knowledge_search`（混合检索，可指定文档与 `top_k`）、`get_document`（长文档分页读取）、`list_tables`、`sql_query`（仅 SELECT）。
- **工具降级**：工具集不可用时（MCP 掉线或未安装）自动收缩，system prompt 随之改写，不会诱导模型调用不存在的工具。
- **上下文预算**：工具结果按工具分档截断（检索 6000 字 / 文档 10000 字），长文档使用 `offset` 分页续读，引文编号在单次回答内全局唯一。
- **文档管理**：PDF/TXT 上传，MD5 判重（内容重复返回 409），切片后向量化入库。
- **认证权限**：JWT（HS256）+ pbkdf2；admin/user 两种角色，上传与重置限 admin。
- **会话持久化**：Agent 多轮记忆落库，历史消息按窗口注入。
- **全异步**：async SQLAlchemy + async OpenAI，IO 全程不阻塞事件循环。
- **评测体系**：104 题检索消融 + 39 题 Agent 评测，指标全部由脚本生成并落盘 JSON（见 [Evaluation](#evaluation)）。
...
- **来源面板编号与答案中的 `[n]` 不一致**：已知偏差，见 [兼容状态](#兼容状态) 最后一行。
...
不采用框架的原因：这条循环承载了 Agent 的全部工程细节——截断保护、超时、迭代上限、
history 滑动窗口、工具异常如何回灌给模型、何时丢弃模型的前置思考。
交由框架处理会将这些决策隐藏于黑盒，出现问题时无从定位。
...
配合两条降级路径：**未安装 `mcp` 包** → `tools.py` 捕获 `ImportError`，仅使用 4 个手写工具；
...
评测代码位于 `eval/`。**README 中的数字全部由脚本生成并落盘为 JSON**，再渲染成表格，
不手写、不估算——无法生成时显示「暂无数据」，不以任何方式编造。
...
5. **本评测集偏向字面匹配，这是其已知缺陷。** 题目由 LLM 依据语料原文生成，
   ...
   要消除这一偏向，需补充一批零字面重叠的改写题（见 Future Work）。
...
**裁判的参照系（这一点决定数字是否可信）**：裁判查看的是 **Agent 该次请求实际获得的工具返回**——
由脚本按 `trace` 中的工具参数**重放同一批调用**得到，走与生产同一套渲染逻辑
（按工具分档的字符预算、结构化结果的逐条裁剪）与**同一套引文编号**，
因此裁判看到的 `[n]` 与 Agent 当时写下的 `[n]` 指向同一份资料。
不能提供整篇文档（会将 Agent 根本未检索到的内容判定为「有据可依」，从而漏判真实幻觉），
也不能只提供检索结果的前若干字符（会将「确实检索到、仅超出窗口」的内容误判为编造）。
拒答判定采用「关键词命中即判定，未命中再交由裁判」，因为模型表示拒绝的措辞每轮不同，仅依靠关键词会漏判。

**稳定复现**：统一口径后连跑 8 次，正确率 14/15、忠实性 15/15、完成率 39/39、工具选择 33/33 次次一致；
引用命中 7 次 18/18、1 次 17/18；拒答补上裁判兜底后 2 次均为 6/6。
（修正前曾出现 11/15、13/15 两种数字，属于重放口径的伪影，并非模型波动。）
**上下文预算修复后**，唯一稳定失败的那题也通过：正确率 15/15，其余指标不变（见下节）。
...
**已知偏差（本轮未实施）**：前端来源面板按数组下标显示 `【i+1】`，未使用事件中的 `id`，
因此多次检索时答案的 `[n]` 与面板序号仍可能不一致。后端已将 `id` 放入 `sources` 事件，
前端修改两行即可对齐（`s.id ?? i+1`）。
...
按优先级排序，均为当前**已明确但尚未实施**的工作：

- **代码级输出脱敏**：目前「不得泄露表名/字段名/SQL」仅为 system prompt 中的一条约束，
  模型仍可能违反。方案是在 executor 输出处做一层确定性替换（内部标识符 → 业务名称），
  并为 `list_tables` 过滤掉 `agent_sessions` / `agent_messages` / `users` 这类内部表，
  最后补充一批脱敏评测用例，使「未泄露」成为可测量的指标。
- **前端来源面板使用 `id`**：答案中的 `[n]` 与面板序号不一致，修改两行即可对齐（`s.id ?? i+1`，
  并在 `types.ts` 的 `Source` 上增加 `id?: number`）。
- **年份策略的全量扫描开销**：将候选集放大到全量是为了让正确年份的文档能进入 top-k，
  代价是 `search_by_vector` 从 49ms 上升至 1456ms（已实测，见 Evaluation 第 3 节）。
  可行方向是使用 Milvus 的标量过滤（`source like "%2024%"`）先缩小候选集再检索，
  而非将全量拉回后在应用层排序——但需先验证过滤后的召回率不下降。
- **纯改写题以消除评测集的词汇偏向**：现有题目由 LLM 从语料原文生成，
  天然复用原文用词，对 BM25 这类词法检索器系统性有利（证据：标注为「主题语义」
  的题目 BM25 达到 100%，向量仅 76.9%）。需补充一批**零字面重叠**的改写题，
  为向量检索提供公平的评测环境。
- **重排序（Rerank）**：在 RRF 之后接一个 cross-encoder 重排，预期可提升「相似文本」题型的区分度。
  未实施的原因是它引入一次额外的模型调用，需先有消融数据证明收益大于延迟代价。
- **检索阈值拒答**：目前拒答由 Agent 判断，可再叠加一层相似度阈值作为双保险——
  前提是先用评测集标定出稳定的阈值，否则阈值本身就会成为新的误判源。
