...
| **BM25 索引走指纹缓存** | 建索引要对全部 chunk 重新分词，是查询路径上的固定开销；按 `(chunk数, 最大id)` 指纹判断语料是否变更，自动失效无需手动清理 |
...
`BM25Index` 构建要对全部 chunk 重新分词，而它原本每次 `rag.search()` 都重建一次，
是查询路径上一笔固定开销。改成按指纹缓存后（本机实测，`python eval/bench_bm25_cache.py` 可复现）：

| 场景 | 耗时 |
|------|-----:|
| 语料 | 34 份文档 → 835 个 chunk（435,400 字） |
| 每次检索重建（原实现） | 276.7 ms |
| 首次构建（冷，等价原开销） | 238.0 ms |
| **命中缓存（改后常态）** | **0.021 ms** |

BM25 这一段降幅约 **1.3 万倍**，每 100 次检索省下约 **27.7 s**。

> 口径说明：这里只测 BM25 那一段。一次完整的 `rag.search()` 还包含 Milvus 向量检索和
> 一次 embedding API 调用，端到端耗时以消融表里的「平均延迟」为准。
