...
- `eval/qa_pairs.json`：**30 题**，含 `gold_keywords`（标准答案文档关键词）与 `expected_year`（期望年份）。
- `eval/evaluate_retrieval.py` 逐题跑 `rag.search()` 统计：
  - **Recall@5 / Recall@8**：标准答案是否进入 top-5 / top-8。
  - **年份 Top-1 命中率**：top-1 是否含正确年份。
  - 平均检索延迟。
- 结果（记录于 CLAUDE.md）：**Recall@8 / Recall@5 = 100%**。面试前建议自己跑一遍确认当前状态。
